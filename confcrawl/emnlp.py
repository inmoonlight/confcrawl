...
def emnlp_2020(save_dir: Optional[str] = None) -> None:
    ...
    if r.ok:
        logger.info(
            "EMNLP 2020 paper crawling successed! Now it is ready to be parsed!"
        )

    root = html.fromstring(r.text)
    long_titles = root.xpath(
        "/html/body/div/div/div/main/article/div/section[2]/section[1]/ul/li/article/span[1]/text()[1]"
    )
    long_authors = root.xpath(
        "/html/body/div/div/div/main/article/div/section[2]/section[1]/ul/li/article/span[2]/text()[1]"
    )
    assert len(long_titles) == len(long_authors)
    short_titles = root.xpath(
        "/html/body/div/div/div/main/article/div/section[2]/section[2]/ul/li/article/span[1]/text()[1]"
    )
    short_authors = root.xpath(
        "/html/body/div/div/div/main/article/div/section[2]/section[2]/ul/li/article/span[2]/text()[1]"
    )
    assert len(short_titles) == len(short_authors)

    for long_title, long_author in tqdm(
        zip(long_titles, long_authors), desc="Long paper", total=len(long_titles)
    ):
    ...
    for short_title, short_author in tqdm(
        zip(short_titles, short_authors), desc="Short paper", total=len(short_titles)
    ):
